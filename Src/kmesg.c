...
RK_ERR kMboxInit( RK_MBOX *const kobj, ADDR const initMailPtr)
{
    RK_CR_AREA
    RK_CR_ENTER
    if (kobj == NULL)
...
RK_ERR kMboxPend( RK_MBOX *const kobj, ADDR *recvPPtr, RK_TICK const timeout)
{
...
/* replace what is stored at the memory location tailPtr points */
/* to with the incoming sendPtr, and advances tailPtr 1 word    */
    *( ULONG**) (kobj->tailPtr ++) = ( ULONG*) sendPtr;
/* inc number of items in the queue */
    kobj->countItems ++;

/* check for wraparound - if there are max items on the queue
...
RK_ERR kQueuePend( RK_QUEUE *const kobj, ADDR *recvPPtr, RK_TICK timeout)
{
    RK_CR_AREA
    RK_CR_ENTER
    if (kobj == NULL || recvPPtr == NULL)
...
RK_ERR kStreamPeek( RK_STREAM *const kobj, const ADDR recvPtr)
{
    RK_CR_AREA
    RK_CR_ENTER
    if ((kobj == NULL) || (recvPtr == NULL) || (kobj->init == 0))
...
RK_ERR kMRMInit( RK_MRM *const kobj, RK_MRM_BUF *mrmPoolPtr, ADDR mesgPoolPtr,
        ULONG nBufs, ULONG dataSizeWords)
{
    RK_CR_AREA
    RK_CR_ENTER
    RK_ERR err = RK_ERROR;
...
/* deallocate if not used and not the curr buf */
        if ((bufPtr->nUsers == 0) && (kobj->currBufPtr != bufPtr))
        {
            ...
            kassert( !kMemFree( &kobj->mrmDataMem, ( ADDR )mrmDataPtr));
            err = kMemFree( &kobj->mrmMem, ( ADDR) bufPtr);
